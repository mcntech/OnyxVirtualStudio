	...
	if(mpVidCapSource) {
		...
		if(mVidSrcFormat == VID_SRC_FORMAT_MJPEG){
			...
			hr = CoCreateInstance(CLSID_Colour, NULL,
					CLSCTX_INPROC, IID_IBaseFilter, (void **)&pDecFilter);
			if(FAILED(hr)) {
				DbgOut(DBG_LVL_ERR,("[DSHOW] Can not create colorspace converter!")); 
				goto Exit;
			}
			...
			if (SUCCEEDED(hr)) {     // Add the filter to the graph.
				...
				if (FAILED(hr)) {
					DbgOut(DBG_LVL_ERR,("[DSHOW] Error in adding Color Space converter to the graph"));
					goto Exit;
				}
			}
		}
		...
		bool horizontalMirroring)
{
	...
	if (strcmp(vidDeviceName.c_str(),  CSTR_DESKTOP_VIDEO) == 0) {
		...
		if(mpVidCapSource) {
			...
			ChooseVidFormat(pGraph,  mpVidCapSource, MEDIASUBTYPE_MJPG, nWidth,nHeight);
			if(ChooseVidFormat(pGraph,  mpVidCapSource, MEDIASUBTYPE_MJPG, nWidth,nHeight) == 0) {
				SetVidVerticalFlip(mpVidCapSource, true);
			} else 
			{
				logMessage("[DSHOW] Can notset MJPEG output type. Trying YUV!"); 
				if((ChooseVidFormat(pGraph,  mpVidCapSource, MEDIASUBTYPE_YUYV, nWidth,nHeight) == 0) || 
					(ChooseVidFormat(pGraph,  mpVidCapSource, MEDIASUBTYPE_IYUV, nWidth,nHeight) == 0) ||
					(ChooseVidFormat(pGraph,  mpVidCapSource, MEDIASUBTYPE_YUY2, nWidth,nHeight) == 0) ||
					(ChooseVidFormat(pGraph,  mpVidCapSource, MEDIASUBTYPE_RGB24, nWidth,nHeight) == 0) ||
					(ChooseVidFormat(pGraph,  mpVidCapSource, MEDIASUBTYPE_UYVY, nWidth,nHeight) == 0) ||
					(ChooseVidFormat(pGraph,  mpVidCapSource, MEDIASUBTYPE_Y41P, nWidth,nHeight) == 0) ||
					(ChooseVidFormat(pGraph,  mpVidCapSource, MEDIASUBTYPE_YV12, nWidth,nHeight) == 0) ||
					(ChooseVidFormat(pGraph,  mpVidCapSource, MEDIASUBTYPE_YUYV, nWidth,nHeight) == 0)) {
						mVidSrcFormat = VID_SRC_FORMAT_YUVX;
				} else {
					logMessage("[DSHOW] Failed to set IYUV, YUY2, YUYV, YUYV output type. "); 
				}
			}
		}
	}
